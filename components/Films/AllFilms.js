...
const AllFilms = () => {
        ...
        if (data != undefined && library != undefined) {
            ...
            let value = data.map((data, index) => {
                if (string_to_slug(data.title).indexOf(string_to_slug(word)) >= 0) {
                    const isInLibrary = library.findIndex(library => library.name === data.title)
                    if (isInLibrary > -1) {
                        var disable = true
                    } else {
                        var disable = false
                    ...
                    return (
                        <FilmsWidget key={index} data={data} disable={disable} />
                    )
                }
            })
    ...
    return (
        <>
            ...
            <input type="text" className="search" placeholder="A New Hope" value={word} onChange={e => setWord(e.target.value)} />
            <div className="row">
                {listOfFilms}
            </div>
        </>
