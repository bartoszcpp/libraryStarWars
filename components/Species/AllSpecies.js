...
const AllSpecies = () => {
    const { species, library } = React.useContext(AppContex);
    const data = species
        ...
        if (data != undefined && library != undefined) {
            ...
            let value = data.map((data, index) => {
                if (string_to_slug(data.name).indexOf(string_to_slug(word)) >= 0) {
                    ...
                    return (
                        <SpeciesWidget key={index} data={data} disable={disable} />
                    )
                }
            })
            setListOfSpecies(value)
        }
    }, [word]);
    ...
    return (
        <>
            ...
            <input type="text" className="search" placeholder="Wookie" value={word} onChange={e => setWord(e.target.value)} />
            <div className="row">
                {listOfSpecies}
            </div>
        </>
