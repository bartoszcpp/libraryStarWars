...
const PeoplePage = (props) => {
    const { name } = props;
    const { people, films, species, planets, starships, vehicles, basedUrl } = React.useContext(AppContex);
    const data = people
    ...
    if (peopleObject != undefined) {
        ...
        var listOfStarships = peopleObject.starships.map((starship, index) => {
            let data = starships.find(dataStarship => dataStarship.url === starship);
            let slugName = string_to_slug(data.name)
            return (
                <p key={index}>
        ...
        let homeworld = planets.find(dataPlanet => dataPlanet.url === peopleObject.homeworld);
                    ...
                    <div className="object">
                        ...
                        <div className="row">
                            <div className="col-md-4">
                                <h2>Data: </h2>
                                <p>Birth Year: <b>{peopleObject.birth_year}</b></p>
                                <p>Eye Color: <b>{peopleObject.eye_color}</b></p>
                                <p>Gender: <b>{peopleObject.gender}</b></p>
                                <p>Hair Color: <b>{peopleObject.hair_color}</b></p>
                                <p>Height: <b>{peopleObject.height}</b></p>
                                <p>
                                    Homeworld:
                                    <a href={`${basedUrl}planets/${slugNameHomeworld}`}> {homeworld.name}</a>
                                </p>
                                <p>Mass: <b>{peopleObject.mass}</b></p>
                                <p>Skin Color: <b>{peopleObject.skin_color}</b></p>
                            </div>
                            <div className="col-md-4">
                                <h2>Films: </h2>
                                {listOfFilms}
