...
const AllStarships = () => {
        ...
        if (data != undefined && library != undefined) {
            ...
            let value = data.map((data, index) => {
                if (string_to_slug(data.name).indexOf(string_to_slug(word)) >= 0) {
                    ...
                    return (
                        <StarshipsWidget key={index} data={data} disable={disable} />
                    )
                }
            })
    ...
    return (
        <>
            ...
            <input type="text" className="search" placeholder="Death Star" value={word} onChange={e => setWord(e.target.value)} />
            <div className="row">
                {listOfStarships}
            </div>
        </>
